...
class AlphaFactorCalculator:
        ...
        for idx, factor_name in enumerate(self.base_factors, 1):
            ...
            try:
                ...
                if family == "MOMENTUM":
                    ...
                    expressions = self._calculate_quantitative_factors(
                        ldf, factor_name, factor_config, available_columns
                    )
                elif family == "CANDLESTICK_PATTERNS":
                    expressions = self._calculate_candlestick_factors(
                        ldf, factor_name, factor_config, available_columns
                    )
                elif family == "CHART_PATTERNS":
                    expressions = self._calculate_chart_pattern_factors(
                        ldf, factor_name, factor_config, available_columns
                    )
                    ...
                    expressions = self._calculate_support_resistance_factors(
                        ldf, factor_name, factor_config, available_columns
                    )
                else:
        ...
        for idx, factor_name in enumerate(self.sophisticated_factors, 1):
            ...
            try:
                ...
                if calculation_type == "regime_analysis":
                    ...
                    expressions = self._get_cross_asset_relative_expressions(
                        factor_name, factor_config, available_columns
                    )
                elif calculation_type == "cross_asset_correlation":
                    expressions = self._get_cross_asset_correlation_expressions(
                        factor_name, factor_config, available_columns
                    )
    ...
    def calculate_sophisticated_factors(self, unified_df: pl.DataFrame) -> pl.DataFrame:
        ...
        try:
            ...
            # Validation
            sophisticated_factors_count = len(
                [col for col in result_df.columns if col.startswith("Factor_") and col in self.sophisticated_factors]
            )
    ...
    def _calculate_momentum_factors(
        self, ldf: pl.LazyFrame, factor_name: str, factor_config: Dict[str, Any], available_columns: set
    ) -> List[pl.Expr]:
        ...
        other_features = [f for f in available_features if f not in rsi_features + stoch_features + williams_features]
    ...
    def _calculate_volume_factors(
        ...
        if "obv_ema" in available_features:
            obv_ema_signal = ((pl.col("obv_ema") > pl.col("obv_ema").shift(1)).cast(pl.Float64)).alias("obv_ema_signal")
        ...
        if len(volume_signals) == 1:
            ...
            total_weight = sum(
                0.3 if i < len(signal_names) and ("obv" in signal_names[i] or "vwap" in signal_names[i]) else 0.2
    ...
    def _calculate_volatility_factors(
        ...
        for feature in available_features:
            if feature.startswith("atr_"):
                ...
                atr_signal = fast_rolling_normalize(pl.col(feature), window=50, fill_value=0.5, min_periods=25).alias(
                    f"{feature}_signal"
                )
        ...
        if all(col in available_features for col in ["bb_upper", "bb_lower", "bb_width"]):
            ...
            bb_width_signal = fast_rolling_normalize(
                pl.col("bb_width"), window=50, fill_value=0.5, min_periods=25
    ...
    def _calculate_support_resistance_factors(
        self, ldf: pl.LazyFrame, factor_name: str, factor_config: Dict[str, Any], available_columns: set
    ) -> List[pl.Expr]:
        """
        ...
        if len(signal_info) == 1:
            ...
            # Normaliser par la somme des poids
            total_weight = sum(0.4 if st == "sr" else 0.3 if st == "trend" else 0.2 for _, _, st in signal_info)
    ...
    def _calculate_quantitative_factors(
        self, ldf: pl.LazyFrame, factor_name: str, factor_config: Dict[str, Any], available_columns: set
    ) -> List[pl.Expr]:
        ...
        if vol_features:
            for feature in vol_features:
                vol_signal = fast_rolling_normalize(pl.col(feature), window=100, fill_value=0.5, min_periods=50).alias(
                    f"{feature}_signal"
                )
        ...
        if entropy_features:
            for feature in entropy_features:
                ...
                entropy_signal = fast_rolling_normalize(
                    pl.col(feature), window=50, fill_value=0.5, min_periods=25
        ...
        persistence_features = [
            f for f in available_features if any(x in f for x in ["persistence", "clustering", "hurst"])
        ]
    ...
    def _calculate_candlestick_factors(
        self, ldf: pl.LazyFrame, factor_name: str, factor_config: Dict[str, Any], available_columns: set
    ) -> List[pl.Expr]:
        ...
        for feature in available_features:
            ...
            if any(
                strong_pattern in feature for strong_pattern in ["engulfing", "hammer", "morning_star", "evening_star"]
            ):
    ...
    def _calculate_chart_pattern_factors(
        self, ldf: pl.LazyFrame, factor_name: str, factor_config: Dict[str, Any], available_columns: set
    ) -> List[pl.Expr]:
    ...
    def _calculate_harmonic_factors(
        self, ldf: pl.LazyFrame, factor_name: str, factor_config: Dict[str, Any], available_columns: set
    ) -> List[pl.Expr]:
    ...
    def _get_regime_expressions(
        self, factor_name: str, factor_config: Dict[str, Any], available_columns: set
    ) -> List[pl.Expr]:
        ...
        if factor_name == "Factor_Regime_Hurst_Score":
            if "quant_hurst_exponent" in available_columns:
                hurst_score = ((pl.col("quant_hurst_exponent") - 0.5) * 2 + 0.5).clip(0.0, 1.0).alias(factor_name)
                return [hurst_score]
            else:
                return [pl.lit(0.5).alias(factor_name)]
        ...
        if factor_name == "Factor_Risk_TailEvent_Score":
            ...
            if risk_signals:
                if len(risk_signals) == 2:
                    tail_risk_score = (risk_signals[0] * 0.3 + risk_signals[1] * 0.7).alias(factor_name)
                else:
    ...
    def _get_persistence_expressions(
        self, factor_name: str, factor_config: Dict[str, Any], available_columns: set
    ) -> List[pl.Expr]:
        ...
        if factor_name == "Factor_Persistence_Score":
            ...
            for feature, weight in zip(autocorr_features, weights):
                if feature in available_columns:
                    autocorr_norm = ((pl.col(feature).clip(-1.0, 1.0) + 1.0) / 2.0 * weight).alias(
                        f"{feature}_weighted"
                    )
    ...
    def _get_cross_asset_relative_expressions(
        self, factor_name: str, factor_config: Dict[str, Any], available_columns: set
    ) -> List[pl.Expr]:
    ...
    def _get_cross_asset_correlation_expressions(
        self, factor_name: str, factor_config: Dict[str, Any], available_columns: set
    ) -> List[pl.Expr]:
    ...
    def _get_second_order_expressions(
        self, factor_name: str, factor_config: Dict[str, Any], available_columns: set
    ) -> List[pl.Expr]:
        ...
        if factor_name == "Factor_Momentum_Acceleration_Score":
            ...
            if "Factor_Momentum_Score" in available_columns:
                ...
                accel_score = fast_rolling_normalize(
                    momentum_accel_raw, window=20, fill_value=0.5, min_periods=10
                ).alias(factor_name)
    ...
    def _get_divergence_expressions(
        self, factor_name: str, factor_config: Dict[str, Any], available_columns: set
    ) -> List[pl.Expr]:
        ...
        if factor_name == "Factor_Divergence_Score_Bullish":
            ...
            if "close" in available_columns and "Factor_Momentum_Score" in available_columns:
                ...
                bullish_div = ((price_change < 0) & (momentum_change > 0)).cast(pl.Float64).alias(factor_name)
            ...
            if "close" in available_columns and "Factor_Momentum_Score" in available_columns:
                ...
                bearish_div = ((price_change > 0) & (momentum_change < 0)).cast(pl.Float64).alias(factor_name)
    ...
    def _get_confluence_expressions(
        self, factor_name: str, factor_config: Dict[str, Any], available_columns: set
    ) -> List[pl.Expr]:
        ...
        if factor_name == "Factor_Trend_Confirmation_Score":
            ...
            if confirmation_signals:
                ...
                confirmation_score = (pl.sum_horizontal(confirmation_signals) / len(confirmation_signals)).alias(
                    factor_name
                )
    ...
    def _calculate_regime_factors(
        self, df: pl.DataFrame, factor_name: str, factor_config: Dict[str, Any]
    ) -> pl.DataFrame:
        """
    ...
    def _calculate_persistence_factors(
        self, df: pl.DataFrame, factor_name: str, factor_config: Dict[str, Any]
    ) -> pl.DataFrame:
        """
    ...
    def _calculate_cross_asset_relative_factors(
        self, df: pl.DataFrame, factor_name: str, factor_config: Dict[str, Any]
    ) -> pl.DataFrame:
        """
        ...
        if factor_name == "Factor_RelativeStrength_Momentum":
            ...
            if len(momentum_cols) >= 2:
                ...
                if btc_momentum and eth_momentum:
                    ...
                    relative_strength = (
                        (pl.col(eth_momentum) / (pl.col(btc_momentum) + 1e-6)).rolling_mean(10).alias(factor_name)
                    )
    ...
    def _calculate_cross_asset_correlation_factors(
        self, df: pl.DataFrame, factor_name: str, factor_config: Dict[str, Any]
    ) -> pl.DataFrame:
        """
    ...
    def _calculate_second_order_factors(
        self, df: pl.DataFrame, factor_name: str, factor_config: Dict[str, Any]
    ) -> pl.DataFrame:
        """
        ...
        if factor_name == "Factor_Momentum_Acceleration_Score":
            if "Factor_Momentum_Score" in df.columns:
                ...
                momentum_accel = (pl.col("Factor_Momentum_Score").diff(5).rolling_mean(3)).alias(factor_name)
            ...
            if "Factor_Volatility_Score" in df.columns:
                ...
                vol_expansion = (pl.col("Factor_Volatility_Score").pct_change(3).rolling_mean(2)).alias(factor_name)

                return df.with_columns([vol_expansion])

        return df.with_columns([pl.lit(0.0).alias(factor_name)])

    def _calculate_divergence_factors(
        self, df: pl.DataFrame, factor_name: str, factor_config: Dict[str, Any]
    ) -> pl.DataFrame:
        """
    ...
    def _calculate_climax_factors(
        ...
        if factor_name == "Factor_Reversal_Climax_Score":
            ...
            if all(col in df.columns for col in ["close", "bb_upper", "bb_lower"]):
                price_extreme = ((pl.col("close") > pl.col("bb_upper")) | (pl.col("close") < pl.col("bb_lower"))).cast(
                    pl.Float32
                )
